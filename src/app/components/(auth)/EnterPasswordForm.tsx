...
export default function EnterPasswordForm() {
  ...
  const handleOnClick = async () => {
    const response = await apiFetch(
      `${process.env.NEXT_PUBLIC_AUTHSERVICE_URL}/auth/login`,
      {
        method: "POST",
        headers: {
  ...
  return (
    <AuthLayout>
      ...
      <button
        className="w-full mt-4 py-3.5 bg-[#E8472A] hover:bg-[#d13d22] active:scale-[0.99] text-white rounded-xl text-sm font-semibold tracking-wide transition-all duration-200"
